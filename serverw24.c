...
#define SERVERW24_PORT 6000
#define SERVERW24_ADDRESS "127.0.0.1"
...
// this variable is being used to track the number of clients connected to the server.
long clientsCount = 0;

// Below I have defined a struct which is being used to store information related to the server.
// It has serverName, serverIP and serverPort fields.
struct Server
{
...
// Next I am defining and array of struct server with 3 server information.
// first is the current server and other two are the mirror server details.
struct Server servers[MAX_SERVERS] = {
    {"ServerW24", "127.0.0.1", 6000},
    {"Mirror1", "127.0.0.1", 7000},
    {"Mirror2", "127.0.0.1", 8000}};
...
int initServer()
{
  ...
  server_address->sin_port = htons(SERVERW24_PORT);
  server_address->sin_addr.s_addr = inet_addr(SERVERW24_ADDRESS);
...
// this is the function to get the serverDetails basically its used by the main server
// to see which server to use to handle the request based on the number of connected clients.
// In this first three clients are conneted to main server.
// next three are connected to mirror1 and next three are connected to mirror 2.
// after that it connects to different servers in alterate order.
struct Server getServerDetails()
{
  if (clientsCount < 4)
  {
    return servers[0];
  }
  else if (clientsCount < 7)
  {
    return servers[1];
  }
...
int main(int argc, char const *argv[])
{
  ...
  {
    printf("ServerW24 Listening at Port: %d and IP: %s\n", SERVERW24_PORT, SERVERW24_ADDRESS);
  }
  ...
  while (1)
  {
    ...
    // receive the request from the client and process its
    char *request;
    int receiveDataStatus = receiveData(client_socket_fd, &request);
    ...
    // check if the request is to check which server to connected.
    if (strcmp("WHICH_SERVER", request) == 0)
    {
      // get the details of the server based on the number of clients connected to the server.
      // further process the details to arrange in a formatted string to be send to the client.
      struct Server server = getServerDetails();
      ssize_t sizeOfResponse = strlen(server.serverIP) + strlen(server.serverName) + 4 + 2;
      char response[sizeOfResponse];
      sprintf(response, "%s:%s:%d:", server.serverName, server.serverIP, server.serverPort);

      // if this is not the server who will handle the request send the correct server details.
      // and further continue listensing to other requests.
      // also increase the client count.
      if ((server.serverPort != SERVERW24_PORT))
      {
        sendData(client_socket_fd, sizeOfResponse, response);
        close(client_socket_fd);
        continue;
      }
      // else just send then details of current server.
      else
      {
        sendData(client_socket_fd, sizeOfResponse, response);
      }
